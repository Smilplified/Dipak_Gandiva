...
export async function PATCH(
  ...
  try {
    ...
    const canEdit = roleNames.includes("qa_tl") || roleNames.includes("admin");
    if (!canEdit) {
      return NextResponse.json(
        { error: "Forbidden: only QA TL can update campaign status" },
        { status: 403 }
      );
    }
    ...
    void logAudit({
      ...
      metadata: { previous_status: prev.status, new_status: status, source: "qatl_campaign_tracker" },
      request,
    });

    return NextResponse.json({ campaign: updated });
  } catch (err) {
    console.error("QATL campaign tracker status update error:", err);
