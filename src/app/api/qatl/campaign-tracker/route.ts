...
async function assertQatlAccess(supabase: Awaited<ReturnType<typeof createClient>>, userId: string) {
  ...
  if (!canView) {
    return {
      error: NextResponse.json(
        { error: "Forbidden: QA TL or Admin role required" },
        { status: 403 }
      ),
    };
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const access = await assertQatlAccess(supabase, user.id);
    ...
    console.error("QATL campaign tracker error:", err);
