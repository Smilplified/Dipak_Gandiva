...
export async function GET(
  ...
  try {
    ...
    if (filesError) {
      console.error("QATL campaign files fetch error:", filesError.message);
    }
    ...
    let leadsList: QatlCampaignLeadRow[] = [];
    let leadsTotal = 0;
    let leadsPage = 1;
    let leadsLimit = 10;

    if (exportAll) {
      const all = await fetchAllQatlCampaignScoredLeads(admin, campaignId);
      leadsList = all.rows;
      ...
      const page = await fetchQatlCampaignScoredLeadsPage(
        admin,
        campaignId,
        parsed.offset,
        parsed.limit
    ...
    if (exportAll) {
      void logAudit({
        ...
        description: `Exported ${leadsTotal.toLocaleString()} scored leads (QA TL campaign detail)`,
        targetType: "campaign",
        targetId: campaignId,
        targetLabel: String(camp.name ?? campaignId),
        metadata: {
          row_count: leadsTotal,
          source: "qatl_campaign_detail",
        },
        request,
      });
    }
    ...
    console.error("QATL campaign detail error:", err);
