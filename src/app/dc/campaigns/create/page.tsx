...
export default function DCCampaignCreatePage() {
  ...
  const calculatedRevenue =
    cpl != null && totalAllocation != null && Number(cpl) >= 0 && Number(totalAllocation) >= 0
  ...
  const submit = async () => {
    try {
      ...
      const res = await fetch("/api/dc/campaigns/create", {
        method: "POST",
      ...
      const filesToUpload = fileList.filter((f) => f.originFileObj).map((f) => f.originFileObj as File);

      if (filesToUpload.length > 0) {
        const { errors: uploadErrors } = await uploadCampaignFilesDirect(
          `/api/dc/campaigns/${campaignId}/files`,
          filesToUpload
        );
      ...
      router.replace(`/dc/campaigns/${campaignId}`);
    } catch (err) {
    ...
    <div style={{ padding: "0 24px 24px", maxWidth: 1200, margin: "0 auto" }}>
      ...
      <Card title="Create Campaign" style={{ marginBottom: 24 }}>
        <Form form={form} layout="vertical" initialValues={{ status: "draft" }}>
              ...
              <Form.Item label="Revenue / Booked (auto)">
                ...
                >
                  {calculatedRevenue != null
                    ? `$${Number(calculatedRevenue).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                    : "—"}
                </div>
          ...
          <Row gutter={24}>
            ...
            <Col xs={24} md={12} lg={8}>
              <Form.Item name="employee_size" label="Employee Size">
                <Select
                  mode="multiple" placeholder="Select employee size ranges"
                  allowClear options={EMPLOYEE_SIZE_OPTIONS}
          ...
          <Row gutter={24}>
            <Col span={24}>
              <Form.Item label="Upload Files" tooltip="PDF, Word, Excel, PowerPoint, CSV, images, ZIP, etc. Max 50MB per file.">
                <Dragger
                  multiple fileList={fileList} accept={ACCEPT_FILE_TYPES}
                  beforeUpload={() => false}
                ...
                >
                  ...
                  <p className="ant-upload-hint">
                    PDF, Word, Excel, CSV, PowerPoint, images, ZIP. Multiple files allowed.
                  </p>
                </Dragger>
              </Form.Item>
            </Col>
