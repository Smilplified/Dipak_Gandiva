...
export default function QATLLeadUploadPage() {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/qatl/lead-upload/campaigns", { credentials: "include" });
        ...
        {loading ? (
          ...
          <Table
            ...
            onRow={(record) => ({
              onClick: () =>
                router.push(
                  `/qatl/campaigns/${record.campaign_id}?delivery_status=delivered`
                ),
