...
export default function CampaignTrackerDashboard() {
  ...
  const hideClientName = hasRole("mis");
  ...
  const { items, pagination, response, isLoading, isFetching, error, refetch } =
    usePaginatedListQuery<RevenueReportClientGroup>({
      queryKeyPrefix: ["mis", "campaign-tracker"],
      url: "/api/mis/campaign-tracker",
      params: listParams,
      listField: "clients",
    });
  ...
  const exportUrl = (format: "csv" | "excel") =>
    buildListApiUrl("/api/mis/campaign-tracker/export", {
      ...listParams,
      page: undefined,
      limit: undefined,
      format,
  ...
  const handleStatusChange = async (campaignId: string, status: string) => {
    ...
    try {
      const res = await fetch(`/api/mis/campaign-tracker/${campaignId}/status`, {
        method: "PATCH",
        credentials: "include",
      ...
      await queryClient.invalidateQueries({ queryKey: ["mis", "campaign-tracker"] });
    } catch (err) {
