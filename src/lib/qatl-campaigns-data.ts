...
async function fetchQatlLeadsForCounts(
  ...
  for (;;) {
    ...
    if (chunk.length < LEADS_PAGE_SIZE) {
      break;
    }
...
export async function loadQatlCampaignsForDateRange(
  ...
  const limit = Math.max(
    1,
    Math.min(maxLimit, options?.limit ?? 10)
  );
  ...
  const [leadTypes, campaignsResult] = await Promise.all([
    ...
    (async () => {
      ...
      if (campaignIdFilter.length > 0) {
        campaignsQuery = campaignsQuery.in(
          "id",
          campaignIdFilter
        );
      }
      ...
      if (leadTypeFilter) {
        ...
        if (safe.length > 0) {
          campaignsQuery = campaignsQuery.ilike(
            "lead_type",
            `%${safe}%`
          );
        }
  ...
  if (campaignsList.length === 0) {
    return {
      ...
      summary: {
        ...
        total_qa_pending_leads: 0,
        total_delivered_leads: 0,
      },
  ...
  const tlByCampaign =
    await fetchBulkCampaignTeamLeaderAssignments(
      supabase,
      campaignsList.map((c) => ({
        id: c.id,
        assigned_team_leader_id:
  ...
  const campaignIds = campaignsList.map(
    (c) => c.id
  );
  ...
  const rawLeads = includeLeads
    ? await fetchQatlLeadsForExport(
        supabase,
        orgId,
        campaignIds,
        uploadRange
      )
    : await fetchQatlLeadsForCounts(
        supabase,
        orgId,
        campaignIds,
        uploadRange
  ...
  for (const c of campaignsList) {
    ...
    // IMPORTANT:
    // Do not display campaigns that have no scored leads
    // in the selected date range.
    //
    // This happens BEFORE pagination so pagination.total
    // and pagination.totalPages remain correct.
    if (leads.length === 0) {
      continue;
    }

    const qaPending = countPendingAuditLeads(
      leads as {
        qa_status?: string | null;
      }[]
    );

    // Calculate Qualified and Disqualified separately
    const qualifiedLeads = leads.filter(
      (lead) =>
        String(lead.qa_status ?? "")
          .toLowerCase()
          .trim() === "qualified"
    ).length;
  ...
  const summary: QatlCampaignsSummary = {
    ...
    total_scored_leads: visible.reduce(
      (sum, c) =>
        sum + (c.scored_leads_count ?? 0),
      0
    ),

    total_qa_pending_leads: visible.reduce(
      (sum, c) =>
        sum + (c.qa_pending_leads_count ?? 0),
      0
    ),

    total_delivered_leads: visible.reduce(
      (sum, c) =>
        sum + (c.delivered_leads_count ?? 0),
      0
    ),
  };

  // IMPORTANT:
  // Pagination happens AFTER zero-lead campaigns
  // have been removed.
  const paged = visible
