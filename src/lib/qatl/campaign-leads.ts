...
export type QatlCampaignLeadRow = {
...
function normalizeQatlCampaignLeadRows(raw: Record<string, unknown>[]): QatlCampaignLeadRow[] {
  return raw.map((row) => ({
    ...(row as QatlCampaignLeadRow),
...
type LeadsQueryResult = {
  rows: QatlCampaignLeadRow[];
  total: number;
  error: { message: string } | null;
};

async function queryQatlCampaignScoredLeadsPage(
  ...
  leadsData = (primary.data ?? null) as unknown as Record<string, unknown>[] | null;
  ...
  if (leadsError && (leadsError.message?.includes("column") || leadsError.message?.includes("disqualification"))) {
    ...
    leadsData = (fallback.data ?? null) as unknown as Record<string, unknown>[] | null;
  ...
  return {
    rows: normalizeQatlCampaignLeadRows((leadsData ?? []) as Record<string, unknown>[]),
    total: leadsCount ?? 0,
    error: null,
  };
}

export async function fetchQatlCampaignScoredLeadsPage(
  admin: SupabaseClient,
  campaignId: string,
  offset: number,
  limit: number
): Promise<{ rows: QatlCampaignLeadRow[]; total: number }> {
  const result = await queryQatlCampaignScoredLeadsPage(admin, campaignId, offset, limit);
  if (result.error) throw new Error(result.error.message);
  return { rows: result.rows, total: result.total };
}

/** Paginated fetch — Supabase returns at most 1000 rows per request. */
export async function fetchAllQatlCampaignScoredLeads(
  admin: SupabaseClient,
  campaignId: string
): Promise<{ rows: QatlCampaignLeadRow[]; total: number }> {
  const all: QatlCampaignLeadRow[] = [];
  let offset = 0;
  let total = 0;

  for (;;) {
    const chunk = await fetchQatlCampaignScoredLeadsPage(
      admin,
      campaignId,
      offset,
      LEADS_PAGE_SIZE
